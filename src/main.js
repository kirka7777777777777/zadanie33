...
Vue.component('task-card', {
    ...
    <div class="task-card">
      ...
      <button v-if="nextStatus" @click="$emit('move-task', task.id, nextStatus )" id="move">
          Move to {{ nextStatus }}
      </button>
...
Vue.component('app', {
    ...
    methods: {
        ...
        moveTask(taskId, newStatus, returnReason = null) {
            const task = this.tasks.find(task => task.id === taskId);
            if (task) {
                if (task.status === 'planned' && newStatus === 'inProgress') {
                    task.status = newStatus;
                } else if (task.status === 'inProgress' && newStatus === 'testing') {
                    task.status = newStatus } else if (task.status === 'testing' && newStatus === 'completed') {
                    task.status = newStatus;
                } else {
                    console.warn(`Cannot move task from ${task.status} to ${newStatus}`);
                    return;
                }
